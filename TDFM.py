...
class TDFM(nn.Module):
    ...
    def __init__(self, n_features, user_df, item_df, time_df, dim=128):
        ...
        total_neigbours = user_df.shape[1] + item_df.shape[1] + time_df.shape[1]
...
def train(epochs=50, batchSize=1024, lr=0.001, dim=128, eva_per_epochs=1, need_eva=True):
    # Read data
    train_triples, test_triples, user_df, item_df, time_df, n_features = \
        DataLoaderKuaiRec.read_data_new()
    # Initialization Model
    net = TDFM(n_features, user_df, item_df, time_df, dim)
